        ...
        try:
            ...
            if not MONGO_DB_URL:
                raise NetworkSecurityException(
                    "MONGO_DB_URL is not set in environment variables", sys
                )
            ...
            logging.info(f"Connected to MongoDB collection: {database_name}.{collection_name}")

            data = list(collection.find())
            if len(data) == 0:
                logging.warning("MongoDB collection is empty: no records found")
